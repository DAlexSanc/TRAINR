...
from core.hef_packager import PackagerWorker

# Default drop location for bundles on the Orin: the host-mounted NVMe folder,
# reachable by the x86 machine that runs the HEF compile.
DEFAULT_BUNDLE_OUT = "/data/host"
...
class OnnxTab(QWidget):
    ...
    def _build(self):
        ...
        _path_row(sg, 2, "Dataset YAML:", "yaml_input",
                  ...
                  lambda: self._browse_file("yaml_input",
                                            "YAML Files (*.yaml *.yml)"))
        _path_row(sg, 3, "Bundle output:", "out_input",
                  DEFAULT_BUNDLE_OUT,
                  lambda: self._browse_dir("out_input"))
        # Seed the default bundle destination if nothing persisted yet.
        if not self.out_input.text().strip():
            self.out_input.setText(DEFAULT_BUNDLE_OUT)
        ...
        pg.addWidget(QLabel("Calib images:"), 1, 0)
        self.calib_count_input = QSpinBox()
        self.calib_count_input.setRange(64, 512)
        self.calib_count_input.setValue(64)
        self.calib_count_input.setToolTip(
            "Number of training images bundled for Hailo quantization "
            "(minimum 64).")
        pg.addWidget(self.calib_count_input, 1, 1)
        ...
        # ── Export button ─────────────────────────────────────────────────
        self._export_btn = QPushButton("Package for HEF")
    ...
    def _on_export(self):
        ...
        self._worker = PackagerWorker(
            onnx_path    = onnx,
            yaml_path    = yaml,
            output_path  = out,
            model_name   = self.model_name_input.text().strip(),
            sample_count = self.calib_count_input.value(),
        )
